...
void findRouteToTarget( gentity_t *self, botTarget_t target ) {
    ...
    for (k = 0; k <= MAX_NODES; ++k) {
        ...
        trap_Trace(&trace, start, NULL, NULL, level.nodes[self->botMind->routeToTarget[startNum]].coord, self->s.number, MASK_DEADSOLID);
        
        //check if we are blocked from getting there
        trap_Trace(&trace2, self->s.pos.trBase, NULL, NULL, level.nodes[self->botMind->routeToTarget[startNum]].coord,self->s.number, MASK_DEADSOLID);
